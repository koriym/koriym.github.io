...
これが現在「RESTful API設計のベストプラクティス」として広く教えられている内容です。Fieldingが定義した統一インターフェースの4つの制約と見比べると、何が残り何が落ちたかが分かります。

1. **リソースの識別** — URLでリソースを表す部分は採用された
2. **表現を通じた操作** — JSONレスポンスとして部分的に残った
3. **自己記述的メッセージ** — 落とされ、代わりにOpenAPIの外部ドキュメントが置かれた
4. **HATEOAS** — 落とされた
